...
Básicamente son POEs (Procedimientos Operativos Estándar) escritos en Markdown, encapsulados en el directorio del agente correspondiente (ej. `agents/[nombre_agente]/directiva/`).
...
Eres el pegamento entre la intención y la ejecución. Ej: no intentas hacer scraping de sitios web tú mismo — lees `agents/[nombre]/directiva/scrape_website.md`, defines entradas/salidas y luego ejecutas `agents/[nombre]/ejecucion/scrape_single_site.py`.

Capa 3: Ejecución (Hacer el trabajo)

Scripts Python deterministas encapsulados en el directorio del agente correspondiente (ej. `agents/[nombre_agente]/ejecucion/`).
...
1. Verifica herramientas primero Antes de escribir un script, revisa el directorio `ejecucion/` dentro de la carpeta local de tu agente según tu directiva. Solo crea nuevos scripts si no existe ninguno en el ámbito del agente.
...
Prueba la herramienta, asegúrate de que funcione.

Actualiza la directiva para incluir el nuevo flujo.

El sistema ahora es más fuerte.
...
Estructura de directorios (Encapsulamiento de Agentes):

`agents/[nombre_agente]/` - Directorio raíz para cada agente (Bounded Context). Autocontenido.
  ├── `directiva/` - POEs en Markdown (el conjunto de instrucciones exclusivas de este agente).
  └── `ejecucion/` - Scripts Python (las herramientas deterministas exclusivas de este agente).
